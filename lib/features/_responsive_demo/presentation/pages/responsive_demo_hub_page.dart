  ...
  Widget build(BuildContext context) {
    final demos = [
      _DemoEntry(
        ...
        onTap: () => unawaited(context.router.push(const ResponsiveDashboardRoute())),
      ),
      _DemoEntry(
      ...
      _DemoEntry(
        ...
        onTap: () => unawaited(context.router.push(const ResponsiveFormRoute())),
      ),
      _DemoEntry(
        title: '自适应网格画廊',
        subtitle: '根据屏幕宽度自动调整列数（2/3/4 列）',
        icon: Icons.grid_view_outlined,
        onTap: () => unawaited(context.router.push(const ResponsiveGalleryRoute())),
      ),
      _DemoEntry(
        title: '设置页分栏',
        subtitle: '手机：分组列表 → 平板：左分类导航 + 右设置项',
        icon: Icons.settings_outlined,
        onTap: () => unawaited(context.router.push(const ResponsiveSettingsRoute())),
      ),
      _DemoEntry(
        title: '图文详情页',
        subtitle: '手机：图上文下 → 平板：图左文右并排',
        icon: Icons.article_outlined,
        onTap: () => unawaited(context.router.push(const ResponsiveArticleRoute())),
      ),
      _DemoEntry(
        title: '登录页',
        subtitle: '手机：全屏表单 → 平板：左品牌展示 + 右登录表单',
        icon: Icons.login_outlined,
        onTap: () => unawaited(context.router.push(const ResponsiveLoginRoute())),
      ),
      _DemoEntry(
        title: '聊天对话',
        subtitle: '手机：全屏对话 → 平板：联系人列表 + 对话窗口',
        icon: Icons.chat_outlined,
        onTap: () => unawaited(context.router.push(const ResponsiveChatRoute())),
      ),
    ];
      ...
      body: ContentConstraint(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (context, index) {
            ...
            return Card(
              child: ListTile(
                leading: Icon(demo.icon, color: Theme.of(context).colorScheme.primary),
                title: Text(demo.title),
                subtitle: Text(demo.subtitle, style: Theme.of(context).textTheme.bodySmall),
