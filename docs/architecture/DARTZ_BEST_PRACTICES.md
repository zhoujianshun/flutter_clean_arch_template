...
class AuthRepositoryImpl implements AuthRepository {
  Future<Either<Failure, User>> getCurrentUser() async {
    // 首先尝试从远程获取
    final remoteResult = await _getRemoteUser();
    
    return remoteResult.fold(
      (failure) async {
        // 如果是网络错误，尝试从缓存获取
        if (failure is NetworkFailure) {
          final cacheResult = await _getCachedUser();
...
abstract class AuthRepository {
  ...
  Future<Either<Failure, User>> phoneLogin({required String phone, required String smsCode});
  Future<Either<Failure, void>> logout();
  Future<Either<Failure, AuthInfo>> getLocalAuthInfo();
}

// ❌ 不好的做法 - 模糊的方法命名
abstract class AuthRepository {
  Future<Either<Failure, User>> get();
  Future<Either<Failure, User>> auth(String phone, String code);
...
class AuthRepositoryImpl implements AuthRepository {
  ...
  Future<Either<Failure, User>> getCurrentUser() async {
    ...
    if (token == null) {
      return const Left(AuthFailure(message: '用户未登录'));
    }
...
group('AuthRepository', () {
  ...
  group('getCurrentUser', () {
    ...
    test('should return Left when not authenticated', () async {
      // Act
      final result = await authRepository.getCurrentUser();
      
      // Assert
      expect(result.isLeft, true);
      expect(result.leftOrNull, isA<AuthFailure>());
      expect(result.leftOrNull!.message, contains('用户未登录'));
    });
  });
});
```
